...
1. **Meeting-document pipeline** (`pipeline.py`, `fetcher.py`, `extractor.py`, `summarizer.py`) — fetches board materials and CAFRs from ~148 U.S. public pension plans, extracts text, summarizes with Claude per-document. Mostly cloud: GHA cron handles 137 of 148 plans daily. The plans in `data/waf_blocked_plans.json` / `data/waf_blocked_cafr_plans.json` are skipped by every cloud run — 8.5% of tracked AUM, though 4 of them had no documents anyway.
   ...
   **"Blocked" means three different things**, established by probe on 2026-09-01 and recorded as `blocked_by` on each entry. Rendering a plan's listing page and downloading the PDFs it links to are independent problems, and a plan is only recoverable if both work:
   - `datacentre_ip` (6 materials, 2 CAFR) — a residential IP gets through on both. These the Mac mini fetches nightly; see the cadence table below. `urs_ut` joined on 2026-09-10: a runner's browser gets an empty listing page where a residential one gets 291 links, the current board documents among them extensionless router links the fetcher now recognises.
...
**Every Claude-calling workflow starts with `python -m scripts.preflight_credit`** (since 2026-09-13): one Haiku call with `max_tokens=1`. An empty prepaid balance or a rejected key fails the job there, before any work, and emails the approval recipient; any other probe error warns and continues. Three silent lapses in September 2026 (09-01, 09-05 to 09-09, 09-11 to 09-12) are why — the pipeline went green producing only `dedup:` rows each time. `tests/test_preflight_credit.py` pins the step's presence and position in all eight workflows, which is also why the three mail variables now sit at job level in each of them.
