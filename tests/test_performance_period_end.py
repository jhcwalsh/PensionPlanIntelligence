...
# --------------------------------------------------------------------------
# Filtering the per-asset-class table by quarter, cell by cell
# --------------------------------------------------------------------------
...
def horizons():
    ...
    cols = ["Quarter", "1 year", "3 year"]
    df = pd.DataFrame([
        {"Plan": "A", "Quarter": 1.0, "1 year": 6.0, "3 year": 9.0,
         "Period end": "2025Q2-2026Q1"},
        {"Plan": "B", "Quarter": 2.0, "1 year": 7.0, "3 year": None,
         "Period end": "2026Q1"},
        {"Plan": "C", "Quarter": None, "1 year": 8.0, "3 year": None,
         "Period end": "2024Q2"},
    ])
    cells = [
        {"Quarter": "2026Q1", "1 year": "2026Q1", "3 year": "2025Q2"},
        {"Quarter": "2026Q1", "1 year": "2026Q1"},
        {"1 year": "2024Q2"},
    ]
    return app_module, df, cols, cells


def test_the_quarter_filter_blanks_cells_not_rows(horizons):
    """Plan A's 3-year figure is a 2025Q2 reading. Keeping the whole row
    because its other cells matched would put a 2025Q2 number on screen under
    a 2026Q1 filter -- the mixing the filter exists to stop."""
    app_module, df, cols, cells = horizons

    out = app_module._keep_only_these_quarters(df, cols, cells, ["2026Q1"])

    a = out[out["Plan"] == "A"].iloc[0]
    assert a["Quarter"] == 1.0 and a["1 year"] == 6.0
    assert a["3 year"] is None or a["3 year"] != a["3 year"]   # blanked


def test_a_row_with_nothing_left_drops_out(horizons):
    """Rather than showing an all-blank row, which reads as a plan that
    reported and did badly."""
    app_module, df, cols, cells = horizons

    out = app_module._keep_only_these_quarters(df, cols, cells, ["2026Q1"])

    assert set(out["Plan"]) == {"A", "B"}


def test_period_end_collapses_to_what_survived(horizons):
    app_module, df, cols, cells = horizons

    out = app_module._keep_only_these_quarters(df, cols, cells, ["2026Q1"])

    assert set(out["Period end"]) == {"2026Q1"}


def test_selecting_two_quarters_keeps_both_and_says_so(horizons):
    app_module, df, cols, cells = horizons

    out = app_module._keep_only_these_quarters(
        df, cols, cells, ["2026Q1", "2025Q2"])

    a = out[out["Plan"] == "A"].iloc[0]
    assert a["3 year"] == 9.0
    assert a["Period end"] == "2025Q2-2026Q1"
    assert set(out["Plan"]) == {"A", "B"}       # C is 2024Q2, still gone


def test_the_filter_does_not_mutate_its_input(horizons):
    """It runs on every rerun against a frame built from cached rows."""
    app_module, df, cols, cells = horizons

    app_module._keep_only_these_quarters(df, cols, cells, ["2026Q1"])

    assert df.loc[0, "3 year"] == 9.0
    assert df.loc[0, "Period end"] == "2025Q2-2026Q1"
