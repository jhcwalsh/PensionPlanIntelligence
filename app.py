...
def _restricted_gate(gate_label: str = "Restricted area") -> bool:
    """Render a password input. Returns True if the user is unlocked.

    Single shared password — set the ``ADMIN_PASSWORD`` environment
    variable on the Render service (or in ``.env`` for local dev).
    The unlocked state is held in ``st.session_state`` so a single
    successful entry covers all gated pages for the session. Browser
    tab close → session ends → re-entry required.

    Fail-open semantics when ``ADMIN_PASSWORD`` is empty/unset, so local
    development without the env var doesn't constantly prompt.
    """
    ...
    with st.form(f"admin_login::{gate_label}", clear_on_submit=True):
        st.markdown(f"**{gate_label} — password required.**")
...
def page_drafts():
    ...
    if not _restricted_gate("Drafts"):
        return
